...
def get_notifications(request):
    ...
    notifications = notifications.all().values('id', 'time_sent', 'subject', 'message').order_by('-time_sent')[:10]
    data = {}
    try:
...
def get_notification_detail(request):
    ...
    doc_id = notification.get_content_object().id if notification.get_content_object() else ''
